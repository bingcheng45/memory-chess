...
`send` and `on` are the raw protocol. `send` issues any CDP command and resolves with its result; `on(listener)` registers `(method, params) => void` for every CDP event the page emits. Use them for facts the DOM cannot show, such as an outgoing request body: `await page.send("Network.enable")`, then `page.on((method, params) => { if (method === "Network.requestWillBeSent") ... })` captures `params.request.postData` for a form the user submits. `send` also reaches domains the wrapped API does not cover, such as `Emulation.setDeviceMetricsOverride` for viewport emulation when checking a layout at phone widths. Prefer the DOM API above for everything the user can see.
