...
describe("LearnArticleRich", () => {
  it("renders quick answer, drill ideas, and reference links for the beginner roadmap", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Memory Chess drill ideas" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "Reference links" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Beginners usually improve fastest/i),
    ).toBeInTheDocument();
  });
  ...
  it("renders the related training paths section for cluster navigation", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Related training paths" }),
    ).toBeInTheDocument();
    expect(
