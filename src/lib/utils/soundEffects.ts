...
export const playSound = (soundName: keyof typeof SOUND_URLS): void => {
  ...
  try {
    ...
    if (!userHasInteracted) {
      console.log(
        "User has not interacted yet, queueing sound to play on next interaction",
      );
      ...
      window.addEventListener("touchstart", playOnNextInteraction, {
        once: true,
      });
    ...
    if (soundName === "timer") {
      ...
      sound.addEventListener(
        "pause",
        () => {
          // Also clear if manually paused elsewhere (e.g., stopTimerSound)
...
export const checkSoundFiles = async (): Promise<void> => {
  ...
  for (const [name, url] of Object.entries(SOUND_URLS)) {
    try {
      ...
      if (!response.ok) {
        console.error(
          `Sound file ${name} (${url}) not found: ${response.status} ${response.statusText}`,
        );
        continue;
      }
      ...
      if (!contentType?.includes("audio")) {
        console.warn(
          `  Warning: ${name} may not be a valid audio file (Content-Type: ${contentType})`,
        );
      }

      if (contentLength && parseInt(contentLength) < 1000) {
        console.warn(
          `  Warning: ${name} is very small (${contentLength} bytes), might be corrupted`,
        );
      }
    } catch (err) {
