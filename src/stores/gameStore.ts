    ...
    (set, get) => ({
      ...
      placePiece: (piece) => {
        ...
        const existingPieceIndex = playerSolution.findIndex(
          (p) =>
            p.position.file === piece.position.file &&
      ...
      isEligibleForLeaderboard: () => {
        ...
        return ["easy", "medium", "hard", "grandmaster"].includes(
      ...
      prepareLeaderboardEntry: (playerName: string) => {
        ...
        return {
          ...
          difficulty: state.config.difficulty as
            | "easy"
            | "medium"
            | "hard"
            | "grandmaster",
          piece_count: state.config.pieceCount,
          correct_pieces: Math.round(
            ((state.accuracy || 0) * state.config.pieceCount) / 100,
          ),
